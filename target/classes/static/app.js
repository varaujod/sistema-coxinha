...
// BUSCAR E POPULAR TELA DE BOAS VINDAS (CLIENTES GIGANTES)
...
function logoutCliente() {
    ...
    // Recarrega lista inicial de clientes (atualizando saldos)
    inicializarTotem();
}

// ATUALIZAR CLIENTE PELO SELECT INTERNO DO PERFIL (Caso usado)
...
function switchKioskSection(sectionName) {
    ...
    // Encontra o botão correto com base no ícone
...
async function carregarPerfilCliente() {
    try {
        ...
        safeText('totem-client-name', cliente.nome.split(' ')[0] || 'Cliente');
...
// HISTÓRICO DE CONSUMO ATIVO (COMPLETED)
async function carregarHistórico() {
    try {
        ...
        // Filtra apenas pedidos ativos (COMPLETED) conforme regra de estorno/remoção
